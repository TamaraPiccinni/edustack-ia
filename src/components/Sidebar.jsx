...
// Añadimos el ícono History
import { BookOpen, FileText, Settings, LayoutDashboard, History } from 'lucide-react';

// Recibimos la prop { historial }
export function Sidebar({ historial }) {
    return (
      ...
      */}
        ...
        */}
                ...
                <div className="pt-6 pb-2">
                    <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2 px-3">
                        <History size={14} />
                        Historial Reciente
                    </h3>
                </div>

                {/* Contenedor con scroll para los items del historial */}
                <div className="space-y-1 overflow-y-auto max-h-64 pr-2 custom-scrollbar">
                    ...
                    {(!historial || historial.length === 0) ? (
                        <p className="text-sm text-gray-500 px-3 italic">No hay documentos recientes.</p>
                    ) : (
                        ...
                        historial.map((item) => (
                            <a
                                key={item.id}
                                href="#"
                                className="block p-3 rounded-lg hover:bg-gray-800 transition-colors border-l-2 border-transparent hover:border-brand"
                            >
                                <p className="text-sm font-medium text-gray-300 truncate">
                                    {item.titulo}
                                </p>
                                <div className="flex items-center justify-between mt-1">
                                    {/* Limpiamos un poco el texto de la acción para que no sea tan largo */}
                                    <span className="text-xs text-brand truncate max-w-[120px]">
                                        {item.accion.split(' ')[0]} {/* Ej: Solo muestra "Resumen" o "Examen" */}
                                    </span>
